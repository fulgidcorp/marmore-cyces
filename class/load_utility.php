...
Class load_utility extends PDO{
	...
	//if(!empty($_REQUEST['show_query']) && $_REQUEST['show_query']=='ogcb0gqzkk'){
	//		echo $sql;
	//	}
    ...
    public function fetchAll($sql, $bind = array()){
        ...
        $data = $stmt->fetchAll($this->_fetchMode);
        return $data;
	...
	public function validateUserInput($array){
       if($array){
           foreach($array as $key=> $val){
             ...
             $array[$key]    = (((str_replace(array('(',')', '=','script',':','\\', '/', '"','>','<','/\/' ,"'",'"','&lt;','&gt;'), '', $val))));
   ...
   function nicetime($date){
    ...
    public function getAjaxSearchQueryOutput($query,$offset,$limit){

            $sql = "SELECT * FROM tbl_search WHERE content LIKE :query OR url LIKE :query OR title LIKE :query OR description LIKE :query LIMIT :limit OFFSET :offset";

            //$bind = array(":query" => $query);
            //$data = $this->fetchAll($sql, $bind);
            ...
            $queryparsed ='%' . $query .'%';
            $stmt->bindValue(':query', $queryparsed);
