...
if($story_title){
	$select     = $objTypes->select("tbl_client_stories", "*", "is_delete = :is_delete AND story_title LIKE :story_title", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_client_stories", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_client_story.php?sysmsg=1001");
	exit();
}
if($story_title){
    $TypeArray = $objTypes->select("tbl_client_stories", "*", "is_delete = :is_delete AND story_title LIKE :story_title", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_client_stories", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Client Story <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_client_story.php"><i class="fa fa-plus-square"></i> Add Story</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="4">
                 <div class="input-group">
                  <input type="text" name="story_title" placeholder="Story Title" class="form-control" value="<?=$story_title?>">
                      <span class="input-group-btn">
                        <button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                        <?php if(isset($story_title) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['story_title'])?></td>
				  	<!--<td><?//=stripslashes($val['story_desc'])?></td>-->
				 ...
				 <!-- </td>-->
                  <td><a href="act_client_story.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_client_story.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_client_story.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_client_story.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_client_story.php';
			document.frmListing.submit();
		}
	}
	else
