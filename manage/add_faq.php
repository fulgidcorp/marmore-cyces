...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> <?=$mode?> FAQ<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_faq.php"><i class="fa  fa-navicon"></i> List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" id="productForm"  method="post" action="act_faq.php" onsubmit="return validateForm();" enctype="multipart/form-data">
          <input type="hidden" name="id" id="id" value="<?=$TypeArray['id']?>"  />
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
                ...
                <textarea class="form-control" id="question" name="question" rows="3" style="width:40%;" placeholder="Question"><?=($TypeArray['question'])?></textarea>
              </div>
              <div class="form-group">
                <label for="exampleInputEmail1">Answer<?=MANDATORY?></label>
                <textarea class="form-control" id="editor1" name="answer"  rows="10" cols="80" placeholder="Description" ><?=stripslashes($TypeArray['answer'])?></textarea>
              </div>
...
jQuery(function(){
    jQuery('.img-wrap2 .close2').click(function() {
        ...
        if(confirm('Are you sure you want to delete selected images?')) {
           	window.location.href = '<?=base_url?>act_faq.php?id='+id+'&param=rimg&prodid=<?php echo $id ?>&pgNo=<?=$_REQUEST['pgNo']?>';
...
function validateForm(){
	if($("textarea#question").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("Question is Mandatory");
		$("input#question").focus();
		return false;
	}
	if($("textarea#editor1").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("Answer is Mandatory");
		$("input#editor1").focus();
		return false;
	}
