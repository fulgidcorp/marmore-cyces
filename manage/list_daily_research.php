...
// }
// if($report_name){
//     	$condition  .=  " AND report_name LIKE '%".$report_name."%'";
// }
if($language){
    	$condition  .=  " AND language LIKE '%".$language."%'";
}
$select	= $objTypes->fetchAll("SELECT * from tbl_daily_research WHERE ".$condition." ORDER BY date_of_upload DESC");
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_daily_search.php?sysmsg=1001");
	exit();
}
if($language){
    	$condition  .=  " AND language LIKE '%".$language."%'";
}
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_daily_research`  WHERE ".$condition." ORDER BY date_of_upload DESC LIMIT ". $limit);

// if($report_name){
//     $TypeArray = $objTypes->select("tbl_daily_research", "*", "is_delete = :is_delete AND report_name LIKE :report_name", $where, "id DESC", $limit);
// }
// else{
//     $TypeArray = $objTypes->select("tbl_daily_research", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
// }	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Daily Research <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_daily_research.php"><i class="fa fa-plus-square"></i> Add Daily Research</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
                        ...
                        <div class="input-group">
        				  <span class="input-group-btn">
        					<button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                             <?php if((isset($language)) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?php if($val['type_of_upload']==1)
				     {
				         echo 'Weekly Wrap';
				     }
				     else if($val['type_of_upload']==2)
				     {
				         echo 'Daily Morning Brief';
				     }
				     else if($val['type_of_upload']==3)
				     {
				         echo 'MENA Fixed Income Daily';
				     }
				     else
				     {
				         echo 'Monthly Market Review (MMR)';
				     }
				  ?></td>
				  <td><?=date("d-m-Y",strtotime($val['date_of_upload']))?></td>
				 ...
				 <!-- </td>-->
                  <td><a href="act_daily_research.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_daily_research.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_daily_research.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_daily_research.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_daily_research.php';
			document.frmListing.submit();
		}
	}
	else
