...
if(($POST['SAVE']=="SAVE")){
	...
	$params = array(
		'overview_desc'	    => $POST['overview_desc'],
		'exp_title'	=> $POST['exp_title'],
		'exp_year'			=> $POST['exp_year'],
		'exp_report_title' => $POST['exp_report_title'],
		'exp_report_count' => $POST['exp_report_count'],
		'is_active'			=>  1,
		'is_delete'			=>  1,
	...
	if($id > 0){
		...
		$update 	= $objTypes->update("tbl_service_overview", $params, "id = :id", $where);
		...
		$insert = $objTypes->insert("tbl_service_overview", $params);
		if($insert){
	...
	if($insert_id > 0){
		...
		if(isset($_FILES['service_img']['name']) && $_FILES['service_img']['name'] != ""){
			...
			if($_FILES['service_img']['size'] > 3097152){
				header("location:add_service_overview.php?sysmsg=16&id=".$insert_id);
                exit();
            }

            if(in_array($ext, $validatefiles) == false){
                header("location:add_service_overview.php?sysmsg=11&id=".$insert_id);
                exit();
            }
			...
			if(move_uploaded_file($_FILES['service_img']['tmp_name'], "../uploads/service_images/".$filename)){
				...
				$update     = $objTypes->update("tbl_service_overview", $img_params, "id = :id", $where);
			}
			else{
				header("location:add_service_overview.php?sysmsg=1003&id=".$insert_id);
				exit();
			}
		}
	...
	header("location:list_service_overview.php?sysmsg=1000&pgNo=".$pgNo);
	exit();
}
...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	$pgNo 		= intval(base64_decode($_REQUEST['pgNo']));
	$title  = $_REQUEST['exp_title'];
    $params     = array("is_active" => $status);
	$where 		= array(":id" => $id);

	$update     = $objTypes->update("tbl_service_overview", $params, "id = :id", $where);
	if($update){
		header("location:list_service_overview.php?sysmsg=1001&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_service_overview.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	$where  = array(":id" => $id);
	$delete = $objTypes->update("tbl_service_overview", $params, "id = :id", $where);
	if($delete){
		header("location:list_service_overview.php?sysmsg=1002&pgNo=$pgNo&title=$title");
		exit();
	}else{
		header("location:list_service_overview.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_service_overview.php?sysmsg=8&pgNo=$pgNo&title=$title");
		exit();
	}
	$Delete	= implode(',', $DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_service_overview SET is_active = '1' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_service_overview.php?sysmsg=1012&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_service_overview.php?sysmsg=13&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_service_overview.php?sysmsg=9&pgNo=$pgNo&title=$title");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_service_overview SET is_active = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_service_overview.php?sysmsg=1013&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_service_overview.php?sysmsg=14&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_service_overview.php?sysmsg=10");
		exit();
	}
	$Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_service_overview SET is_delete = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_service_overview.php?sysmsg=1014&title=$title");
		exit();
	}
    else{
		header("location:list_service_overview.php?sysmsg=4&title=$title");
		exit();
	}
}
