...
//category only
if(isset($_POST['category']) && empty($_POST['country']) && empty($_POST['subcategory']))
{
$report = "'".implode("','", $_POST['category'])."'";
$report_list   = "select * from tbl_report_data where is_active=1 and is_delete=1 and report_category1 IN ($report) order by report_date DESC";
// echo $report_list;exit;
...
if(isset($_POST['country']) && isset($_POST['category']))
{
    $report = "'".implode("','", $_POST['category'])."'";
    $country = "'".implode("','", $_POST['country'])."'";
    $report_list   = "select * from tbl_report_data where is_active=1 and is_delete=1 and report_country IN ($country) and report_category1 IN ($report) order by report_date DESC";
                    ...
                    <div class="row relative-report blogs">';
                          if(isset($TypeArray) && !empty($TypeArray))
                          {
                             foreach($TypeArray as $reports)
                             {
                                 ...
                                 if(isset($reports['report_images']) && !isset($blog_image['dirname']))
                                  {
