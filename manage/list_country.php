...
if($country_name){
	$select     = $objTypes->select("tbl_country", "*", "is_delete = :is_delete AND country_name LIKE :country_name", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_country", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_country.php?sysmsg=1001");
	exit();
}
if($country_name){
    $TypeArray = $objTypes->select("tbl_country", "*", "is_delete = :is_delete AND country_name LIKE :country_name", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_country", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Country <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_country.php"><i class="fa fa-plus-square"></i> Add Country</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="4">
                 <div class="input-group">
                  <input type="text" name="country_name" placeholder="Country Name" class="form-control" value="<?=$country_name?>">
                      <span class="input-group-btn">
                        <button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                        <?php if(isset($country_name) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['country_name'])?></td>
				 <!--   <td align="center">                  -->
				 ...
				 <!-- </td>-->
                  <td><a href="act_country.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_country.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_country.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_country.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_country.php';
			document.frmListing.submit();
		}
	}
	else
