...
if($blog_title){
	$select     = $objTypes->select("tbl_blogs", "*", "is_delete = :is_delete AND blog_title LIKE :blog_title", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_blogs", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_blogs.php?sysmsg=1001");
	exit();
}
if($blog_title){
    $TypeArray = $objTypes->select("tbl_blogs", "*", "is_delete = :is_delete AND blog_title LIKE :blog_title", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_blogs", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="add_blog.php"><i class="fa fa-plus-square"></i> Add Blog</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="3">
                 <div class="input-group">
                  <input type="text" name="blog_title" placeholder="Blog Title" class="form-control" value="<?=$blog_title?>">
                      <span class="input-group-btn">
                        <button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                        <?php if(isset($_POST['blog_title']) && isset($_POST['search'])) { ?>
        					   <button type="reset" class="btn btn-primary btn-flat"><a href="list_blogs.php" style="color:white !important">Reset</a></button>
        					<?php } else{ ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['blog_title'])?></td>
                  <?php
                   $blog_image = pathinfo($val['blog_img']);
                  if($val['blog_img'] != "" && isset($val['blog_img']) && $blog_image['dirname']=='.'){
                      $blog_img ='<img src="../uploads/blog_images/'.$val['blog_img'].'" width="100px"  />';
                  }
                  else if($val['blog_img'] != "" && dirname($val['blog_img']))
                  {
                    $blog_img ='<img src='.$val['blog_img'].' width="100px"  />';
                  }
                  else
                  {
                      $blog_img ='<img src="../uploads/blog_images/Group 160532.png" width="100px"  />';
                  }
                  ?>
                  <td><?= $blog_img ?></td>
				  ...
				  <!--</td>-->
                  <td><a href="act_blog.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_blog.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_blog.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_blog.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_blog.php';
			document.frmListing.submit();
		}
	}
	else
