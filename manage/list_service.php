...
if($service_head){
	$where[':service_head'] = '%'.$service_head.'%';
}


if($service_head){
	$select     = $objTypes->select("tbl_service", "*", "is_delete = :is_delete AND service_head LIKE :service_head", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_service", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($service_head){
    $TypeArray = $objTypes->select("tbl_service", "*", "is_delete = :is_delete AND service_head LIKE :service_head", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_service", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Service <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_service.php"><i class="fa fa-plus-square"></i> Add Service</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="8">
                 <div class="input-group">
                  <input type="text" name="service_head" placeholder="Service Title..." class="form-control" value="<?=$service_head?>">
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['service_head'])?></td>

                  <td><?php
                  if($val['service_img'] != ""){
                      ?>
                      <img src="../uploads/service_images/<?php echo $val['service_img']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
                  ...
                  <td><a href="act_service.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_service.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_service.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_service.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_service.php';
			document.frmListing.submit();
		}
	}
	else
