...
if($news_name){
    	$condition  .=  " AND news_name LIKE '%".$news_name."%'";
}
if($language){
    	$condition  .=  " AND language LIKE '%".$language."%'";
}
$select	= $objTypes->fetchAll("SELECT * from tbl_news WHERE ".$condition." ORDER BY id DESC");
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_news.php?sysmsg=1001");
	exit();
}
if($news_name){
    	$condition  .=  " AND news_name LIKE '%".$news_name."%'";
...
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_news`  WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List News <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_news.php"><i class="fa fa-plus-square"></i> Add News</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <td colspan="3">
                 <div class="input-group">
                  <input type="text" name="news_name" placeholder="News Name" class="form-control" value="<?=$news_name?>">
                </div>
                </td>
                <td colspan="1">
                        <div class="input-group">
                        ...
                        <div class="input-group">
        				  <span class="input-group-btn">
        					<button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                             <?php if((isset($news_name) || isset($language)) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['news_name'])?></td>
				 ...
				 <!-- </td>-->
                  <td><a href="act_news.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_news.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_news.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_news.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_news.php';
			document.frmListing.submit();
		}
	}
	else
