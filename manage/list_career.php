...
if($position){
	$select     = $objTypes->select("tbl_career_jobs", "*", "is_delete = :is_delete AND position LIKE :position", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_career_jobs", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_career.php?sysmsg=1001");
	exit();
}
if($position){
    $TypeArray = $objTypes->select("tbl_career_jobs", "*", "is_delete = :is_delete AND position LIKE :position", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_career_jobs", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
              ...
              <tr>
                <th colspan="4">
                 <div class="input-group">
                  <input type="text" name="position" placeholder="Position" class="form-control" value="<?=$position?>">
                      <span class="input-group-btn">
                        <button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                        <?php if((isset($position)) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
				  ...
				  <td><?=stripslashes($val['position'])?></td>
				  	<td><?=stripslashes($val['position_type'])?></td>
				  	<td><?=stripslashes($val['work_type'])?></td>
				 <!--   <td align="center">                  -->
				 ...
				 <!-- </td>-->
                  <td><a href="act_career.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                  ...
                  <a href="act_career.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
