...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT * FROM tbl_country WHERE id = :id", $params);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> <?=$mode?> Country <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_country.php"><i class="fa  fa-navicon"></i> List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" id="productForm"  method="post" action="act_country.php" onsubmit="return validateForm();" enctype="multipart/form-data">
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
			  <div class="form-group">
                ...
                <input type="text" class="form-control " id="country_name" name="country_name" value="<?=stripslashes($TypeArray['country_name'])?>" placeholder="Country Name" style="width:40%;">
              </div>
			  <div class="form-group">
                  <label for="exampleInputEmail1">Country Image<?=MANDATORY?></label>
                  <input type="file" class="form-control " id="country_image" name="country_image" value="" placeholder="Report Name" style="width:40%;">
                  ...
                  <?php if($TypeArray['country_image']){ ?>
                <a href="#"><img src="../uploads/country_image/<?=stripslashes($TypeArray['country_image'])?>"  onerror="this.style.display='none'" height="80" width="100" onclick='window.open("../uploads/country_image/<?=stripslashes($TypeArray['country_image'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
				<?php } ?>
              </div>
            <!-- /.box-body -->
            <div class="box-footer">
              <button type="submit" class="btn btn-primary" value="SAVE" name="SAVE" id="SAVE">Submit</button>
              <a href="list_country.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
jQuery(function(){
    jQuery('.img-wrap2 .close2').click(function() {
        ...
        if(confirm('Are you sure you want to delete selected images?')) {
           	window.location.href = '<?=base_url?>act_country.php?id='+id+'&param=rimg&prodid=<?php echo $id ?>&pgNo=<?=$_REQUEST['pgNo']?>';
