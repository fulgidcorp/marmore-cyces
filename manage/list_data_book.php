...
$country_name	    = $POST['country_id'];
$databook_name	    = $POST['data_book_name'];
...
if($databook_name){
	$where[':data_book_name'] = '%'.$databook_name.'%';
}
if($country_name){
    	$condition  .=  " AND country_id LIKE '%".$country_name."%'";
}
if($databook_name){
    	$condition  .=  " AND data_book_name LIKE '%".$databook_name."%'";
}
if($language){
    	$condition  .=  " AND language LIKE '%".$language."%'";
}
$select	= $objTypes->fetchAll("SELECT * from tbl_data_book WHERE ".$condition." ORDER BY id DESC");

// if($report_name){
// 	$select     = $objTypes->select("tbl_data_book", "*", "is_delete = :is_delete AND report_name LIKE :report_name", $where, "id DESC");
// }
// else{
// 	$select     = $objTypes->select("tbl_data_book", "*", "is_delete = :is_delete", $where, "id DESC");
// }
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_data_book.php?sysmsg=1001");
	exit();
}
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_data_book`  WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);

// if($report_name){
//     $TypeArray = $objTypes->select("tbl_data_book", "*", "is_delete = :is_delete AND report_name LIKE :report_name", $where, "id DESC", $limit);
// }
// else{
//     $TypeArray = $objTypes->select("tbl_data_book", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
// }	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Databook <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_data_book.php"><i class="fa fa-plus-square"></i> Add Databook</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <td colspan="3">
                 <div class="input-group">
                  <input type="text" name="data_book_name" placeholder="Databook Name" class="form-control" value="<?=$databook_name?>">
                </div>
                </td>
                <td colspan="1">
                        <div class="input-group">
                        ...
                        <div class="input-group">
        				  <span class="input-group-btn">
        					<button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                             <?php if((isset($databook_name) || isset($language)) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['data_book_name'])?></td>

                  <td><?php
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/databook_images/<?php echo $val['image']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
				 ...
				 <!-- </td>-->
                  <td><a href="act_data_book.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_data_book.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_data_book.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_data_book.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_data_book.php';
			document.frmListing.submit();
		}
	}
	else
