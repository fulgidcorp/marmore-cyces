...
if(($POST['SAVE']=="SAVE")){
	...
	if($id > 0){
		...
		$update 	= $objTypes->update("tbl_bulletin", $params, "id = :id", $where);
		...
		$insert = $objTypes->insert("tbl_bulletin", $params);
		if($insert){
        ...
        if(isset($_FILES['book_pdf']['name']) && $_FILES['book_pdf']['name'] != "")
			{			
				...
				if(in_array($ext, $validatefiles) == false){
					header("location:add_bulletin.php?sysmsg=35");
					exit();		
				}
				...
				$updatepdf 		= $objTypes->update("tbl_bulletin", $params, "id = :id", $where);
			}	
		
			}

	header("location:list_bulletin.php?sysmsg=1000&pgNo=".$pgNo);
	exit();
}
...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	$update     = $objTypes->update("tbl_bulletin", $params, "id = :id", $where);
	if($update){
		header("location:list_bulletin.php?sysmsg=1001&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_bulletin.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	$where  = array(":id" => $id);
	$delete = $objTypes->update("tbl_bulletin", $params, "id = :id", $where);
	if($delete){
		header("location:list_bulletin.php?sysmsg=1002&pgNo=$pgNo&title=$title");
		exit();
	}else{
		header("location:list_bulletin.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_bulletin.php?sysmsg=8&pgNo=$pgNo&title=$title");
		exit();
	}
	$Delete	= implode(',', $DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_bulletin SET is_active = '1' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_bulletin.php?sysmsg=1012&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_bulletin.php?sysmsg=13&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_bulletin.php?sysmsg=9&pgNo=$pgNo&title=$title");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_bulletin SET is_active = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_bulletin.php?sysmsg=1013&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_bulletin.php?sysmsg=14&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_bulletin.php?sysmsg=10");
		exit();
	}
	$Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_bulletin SET is_delete = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_bulletin.php?sysmsg=1014&title=$title");
		exit();
	}
    else{
		header("location:list_bulletin.php?sysmsg=4&title=$title");
		exit();
	}
}
