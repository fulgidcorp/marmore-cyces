...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT * FROM tbl_bulletin WHERE id = :id", $params);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> <?=$mode?> Bulletin<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_bulletin.php"><i class="fa  fa-navicon"></i> List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" id="productForm"  method="post" action="act_bulletin.php" onsubmit="return validateForm();" enctype="multipart/form-data">
          <input type="hidden" name="id" id="id" value="<?=$TypeArray['id']?>"  />
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
              ...
              <div class="form-group">
                <label for="exampleInputEmail1">Bulletin Name<?=MANDATORY?></label>
                <input type="text" class="form-control " id="gcc_name" name="gcc_name" value="<?=stripslashes($TypeArray['gcc_name'])?>" placeholder="Bulletin Name" style="width:40%;">
              </div>
              
              <div class="form-group">
                  <label for="exampleInputEmail1">Book Pdf<?=MANDATORY?></label>
                  <input type="file" class="form-control " id="book_pdf" name="book_pdf" value="" placeholder="Book Pdf" style="width:40%;"  multiple="multiple">
                ...
                <a href="#"><img src="../uploads/book_pdf/<?=stripslashes($TypeArray['book_pdf'])?>"  onerror="this.style.display='none'" height="80" width="100" onclick='window.open("../uploads/book_pdf/<?=stripslashes($TypeArray['book_pdf'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
				<?php } ?>
              </div>
            <div class="form-group">
              ...
              <div class="form-group">
                ...
                <input type="text" class="form-control " id="slug" name="report_slug" value="<?=stripslashes($TypeArray['slug'])?>" placeholder="Slug" style="width:40%;">
              </div>
              <div class="form-group">
            ...
            <div class="box-footer">
              ...
              <a href="list_bulletin.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
function validateForm(){
	...
	if($("input#gcc_name").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("Bulletin Name is Mandatory");
