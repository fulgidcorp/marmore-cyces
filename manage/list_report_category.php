...
if($report_category_name){
	$where[':report_category_name'] = '%'.$report_category_name.'%';
}
if($language){
	$where[':language'] = '%'.$language.'%';
}
if($report_category_name){
    	$condition  .=  " AND report_category_name LIKE '%".$report_category_name."%'";
}
if($language){
    	$condition  .= " AND language LIKE '%".$language."%'";
}
$select	= $objTypes->fetchAll("SELECT * from tbl_report_category WHERE ".$condition." ORDER BY id DESC");
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_report_category.php?sysmsg=1001");
	exit();
}
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_report_category`  WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Report Category <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_report_category.php"><i class="fa fa-plus-square"></i> Add Report Category</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              <thead>
                  <tr>
                    <td colspan="3">
                     <div class="input-group">
                        <input type="text" name="report_category_name" placeholder="Category name" class="form-control" value="<?=$report_category_name?>">
                     </div>
                    </td>
                    <td colspan="1">
                        <div class="input-group">
                ...
                <tr>
                  ...
                  <th width="3%">#</th>
                  <th width="10%">Report Category Name</th>
                  <th width="10%">SEO Title</th>
                  <th width="5%">Language</th>
				  <th width="8%"><input type="submit" name="setpriority" id="setpriority" class="btn btn-block btn-danger btn-sm" value="Sort Order" style="width:50%;margin: auto;"></th>
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_report_category.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_report_category.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_report_category.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_report_category.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_report_category.php';
			document.frmListing.submit();
		}
	}
	else
