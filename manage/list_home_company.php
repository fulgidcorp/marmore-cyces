...
if($company_name){
	$select     = $objTypes->select("tbl_home_companies", "*", "is_delete = :is_delete AND company_name LIKE :company_name", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_home_companies", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	for($i=0;$i<$total;$i++){   
		if(($POST["sort_order".$i]>=0)&&($POST['sortorder_id'.$i]>0)){
			$params = array('sort_order' => $POST["sort_order".$i]); 
			$where  = array(':id' => $POST['sortorder_id'.$i]);
			$update = $objTypes->update("tbl_home_companies", $params, "id = :id", $where);	
		}
	} 
	header("location:list_home_company.php?sysmsg=1001");
	exit();
}
if($company_name){
    $TypeArray = $objTypes->select("tbl_home_companies", "*", "is_delete = :is_delete AND company_name LIKE :company_name", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_home_companies", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Home Company <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_home_company.php"><i class="fa fa-plus-square"></i> Add Company</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="8">
                 <div class="input-group">
                  <input type="text" name="company_name" placeholder="Title..." class="form-control" value="<?=$company_name?>">
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['company_name'])?></td>
				  <td><?php
                  if($val['company_img'] != ""){
                      ?>
                      <img src="../uploads/company_images/<?php echo $val['company_img']; ?>" width="100px"  />
                      <?php
                  }
                  ?></td>
				    <td align="center">                  
                  ...
                  <td><a href="act_home_company.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_home_company.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_home_company.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_home_company.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_home_company.php';
			document.frmListing.submit();
		}
	}
	else
