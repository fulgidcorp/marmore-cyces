...
$select	= $objTypes->fetchAll("SELECT * from tbl_report_data WHERE ".$condition." ORDER BY id DESC");
...
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_report_data`  WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Report <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_report.php"><i class="fa fa-plus-square"></i> Add Report</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <td colspan="3">
                 <div class="input-group">
                  <input type="text" name="report_name" placeholder="Report Name" class="form-control" value="<?=$report_name?>">
                </div>
                </td>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['report_name'])?></td>

                  <td><?php
                  if($val['report_images'] != ""){
                       $blog_image = pathinfo($val['report_images']);
                                    
                     if(isset($val['report_images']) && isset($blog_image['dirname']) && $blog_image['dirname']=='.')
                      {
                          $img = '../uploads/report_images/'.$val['report_images'];
                      }
                      else if(isset($val['report_images']) && isset($blog_image['dirname']))
                      {
                          $img = $val['report_images'];
                      }
                      else
                      {
				  ...
				  <!--</td>-->
                  <td><a href="act_report.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_report.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_report.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_report.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_report.php';
			document.frmListing.submit();
		}
	}
	else
