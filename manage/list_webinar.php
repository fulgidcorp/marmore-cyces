...
if($webinar_name){
    	$condition  .=  " AND webinar_name LIKE '%".$webinar_name."%'";
}
if($language){
    	$condition  .=  " AND language LIKE '%".$language."%'";
}
$select	= $objTypes->fetchAll("SELECT * from tbl_webinar WHERE ".$condition." ORDER BY id DESC");
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_webinar.php?sysmsg=1001");
	exit();
}
$TypeArray	= $objTypes->fetchAll("SELECT  * FROM `tbl_webinar`  WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Webinar <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_webinar.php"><i class="fa fa-plus-square"></i> Add Webinar</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <td colspan="3">
                 <div class="input-group">
                  <input type="text" name="webinar_name" placeholder="Webinar Name" class="form-control" value="<?=$webinar_name?>">
                </div>
                </td>
                <td colspan="1">
                        <div class="input-group">
                        ...
                        <div class="input-group">
        				  <span class="input-group-btn">
        					<button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                             <?php if((isset($webinar_name) || isset($language)) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['webinar_name'])?></td>
				  	<td><?php echo date("F d, Y",strtotime($val['webinar_date'])) ?></td>
				 ...
				 <!-- </td>-->
                  <td><a href="act_webinar.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  
                  <a href="add_webinar.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_webinar.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_webinar.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_webinar.php';
			document.frmListing.submit();
		}
	}
	else
