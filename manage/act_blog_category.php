...
if(($POST['SAVE']=="SAVE")){
	...
	$params = array(
		...
		'seo_category_name'	    => $POST['seo_category_name'],
		'seo_category_desc'	    => $POST['seo_category_desc'],
		'is_active'			=>  1,
		'is_delete'			=>  1,
	...
	if($id > 0){
		...
		$update 	= $objTypes->update("tbl_blogs_category", $params, "id = :id", $where);
		...
		$insert = $objTypes->insert("tbl_blogs_category", $params);
		if($insert){
	...
	header("location:list_blog_category.php?sysmsg=1000&pgNo=".$pgNo);
	exit();
}
...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	$update     = $objTypes->update("tbl_blogs_category", $params, "id = :id", $where);
	if($update){
		header("location:list_blog_category.php?sysmsg=1001&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_blog_category.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	$where  = array(":id" => $id);
	$delete = $objTypes->update("tbl_blogs_category", $params, "id = :id", $where);
	if($delete){
		header("location:list_blog_category.php?sysmsg=1002&pgNo=$pgNo&title=$title");
		exit();
	}else{
		header("location:list_blog_category.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_blog_category.php?sysmsg=8&pgNo=$pgNo&title=$title");
		exit();
	}
	$Delete	= implode(',', $DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_blogs_category SET is_active = '1' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_blog_category.php?sysmsg=1012&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_blog_category.php?sysmsg=13&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_blog_category.php?sysmsg=9&pgNo=$pgNo&title=$title");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_blogs_category SET is_active = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_blog_category.php?sysmsg=1013&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_blog_category.php?sysmsg=14&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_blog_category.php?sysmsg=10");
		exit();
	}
	$Delete = implode(',',$DelCheckBox);
	$update = $objTypes->inquery("UPDATE tbl_blogs_category SET is_delete = '0' WHERE id IN ($Delete)");

	if($update > 0){
		header("location:list_blog_category.php?sysmsg=1014&title=$title");
		exit();
	}
    else{
		header("location:list_blog_category.php?sysmsg=4&title=$title");
		exit();
	}
}
