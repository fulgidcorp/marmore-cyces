...
if($question){
	$select     = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete AND question LIKE :question", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_faq.php?sysmsg=1001");
	exit();
}
if($question){
    $TypeArray = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete AND question LIKE :question", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}	
?>
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                <th colspan="3">
                    <div class="input-group">
                        <input type="text" name="question" placeholder="Question" class="form-control" value="<?=$question?>">
                        <span class="input-group-btn">
                            <button type="submit" class="btn btn-success btn-flat" name="search">Search</button>
                            <?php if(isset($question) && isset($_POST['search'])) { ?>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['question'])?></td>
