...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1>Feature Report<small></small> </h1>
  </section>
  <!-- Main content -->
  ...
  <form action="act_feature_report.php" method="post">
    <div class="container">  
  <p>Report 1</p>
  <select id="report1"  name="report1" onmousedown="if(this.options.length>8){this.size=8;}"  onchange='this.size=0;' onblur="this.size=0;" >  

  <?php
  $query="SELECT * from tbl_report_data join tbl_feature_report on tbl_report_data.id=tbl_feature_report.report1 ORDER BY tbl_feature_report.id DESC limit 1";
 ...
 <option value="<?=$row['report1'];?>" ><?=$row['report_name'];?></option>
   <?php }?>
    <?php
                    ...
                    if(sizeof($ProdArray) > 0){
                        foreach($ProdArray as $prod_v){
                            if($prod_v['id'] == $TypeArray['report_name']){
                                $selected = 'selected';
                            }
                            else{
                                $selected = '';
    ...
    <div class="form-group">
            ...
            <?php
                $query ="SELECT *from tbl_report_data join tbl_feature_report on tbl_report_data.id=tbl_feature_report.report2 ORDER BY tbl_feature_report.id DESC limit 1";
                $select_query=mysqli_query($con,$query);
                while($row=mysqli_fetch_assoc($select_query))
                {
            ?>
    ...
    <div class="form-group">
    <p>Report 3</p>
    <select id="report3" name="report3" onmousedown="if(this.options.length>8){this.size=8;}"  onchange='this.size=0;' onblur="this.size=0;">  

          <?php
                $query ="SELECT * from tbl_report_data join tbl_feature_report on tbl_report_data.id=tbl_feature_report.report3 ORDER BY tbl_feature_report.id DESC limit 1";
                $select_query=mysqli_query($con,$query);
                while($row=mysqli_fetch_assoc($select_query))
                {
            ?>
            <option value="<?=$row['report3'];?>" ><?=$row['report_name'];?></option>
          <?php }?>
        <?php
                     $ProdArray  = $objTypes->fetchAll("SELECT report_name,id FROM tbl_report_data");
                    if(sizeof($ProdArray) > 0){
