...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT * FROM tbl_news WHERE id = :id", $params);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> <?=$mode?> News<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_news.php"><i class="fa  fa-navicon"></i> List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" id="productForm"  method="post" action="act_news.php" onsubmit="return validateForm();" enctype="multipart/form-data">
          <input type="hidden" name="id" id="id" value="<?=$TypeArray['id']?>"  />
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
              ...
              <div class="form-group">
                <label for="exampleInputEmail1">News Name<?=MANDATORY?></label>
                <input type="text" class="form-control " id="news_name" name="news_name" value="<?=stripslashes($TypeArray['news_name'])?>" placeholder="News Name" style="width:40%;">
              </div>
              <div class="form-group">
                <label for="exampleInputEmail1">News Link<?=MANDATORY?></label>
                <input type="text" class="form-control " id="news_link" name="news_link" value="<?=stripslashes($TypeArray['news_link'])?>" placeholder="Media Name" style="width:40%;">
              </div>
              <div class="form-group">
                <label for="exampleInputEmail1">Published Date<?=MANDATORY?></label>
                <div class="input-group date" style="width:40%;">
                  <div class="input-group-addon">
                    <i class="fa fa-calendar"></i>
                  </div>
                  <input type="text" class="form-control pull-right" id="created_date" name="created_date" value="<?=stripslashes($TypeArray['created_date'])?>"  placeholder="Published date">
                </div>
              </div>
              <div class="form-group">
                <label for="exampleInputEmail1">Category<?=MANDATORY?></label>
                <input type="text" class="form-control " id="category" name="category" value="<?=stripslashes($TypeArray['category'])?>" placeholder="Category Name" style="width:40%;">
              </div>
              <div class="form-group">
                  <label for="exampleInputEmail1">News Image</label>
                  <input type="file" class="form-control " id="media" name="media" value="" placeholder="News Image" style="width:40%;"  multiple="multiple">
                  ...
                  <?php if($TypeArray['media']){ ?>
                <a href="#"><img src="../uploads/news_images/<?=stripslashes($TypeArray['media'])?>"  onerror="this.style.display='none'" height="80" width="100" onclick='window.open("../uploads/news_images/<?=stripslashes($TypeArray['media'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
				<?php } ?>
              </div>
              <div class="form-group">
            ...
            <div class="box-footer">
              ...
              <a href="list_news.php?&pgNo=<?php echo intval(base64_decode($_REQUEST['pgNo'])); ?>" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
function validateForm(){
	...
	if($("input#news_name").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("News Name is Mandatory");
		$("input#news_name").focus();
		return false;
	}
	if($("input#news_link").val()==""){
		$(".errorDiv").show().fadeOut(4000);
		$('#errormessage').text("News Link is Mandatory");
		$("input#news_link").focus();
		return false;
	}
